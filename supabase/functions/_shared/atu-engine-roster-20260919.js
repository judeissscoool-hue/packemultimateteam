...
import * as modern from './roster-20260919/modern-engine.js';
import * as history from './roster-20260919/history-engine.js';
...
export function isClassicRulesVersion(version) {
  return [legacy,priorModern,priorHistory,previousModern,previousHistory,modern,history].some(e=>e.CLASSIC_RULES_VERSION===version);
}
export function isLegacyRulesVersion(version) { return engines.get(version)===legacy; }
export function legacyView(version) { return isLegacyRulesVersion(version)?legacyPreview:[priorModern,priorHistory].includes(engines.get(version))?priorPreview:[previousModern,previousHistory].includes(engines.get(version))?previousPreview:null; }
...
function poolForRules(version) { return [history,priorHistory,previousHistory].includes(engines.get(version))?'history':engines.get(version)===legacy?'legacy':'modern'; }
