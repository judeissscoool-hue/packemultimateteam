import * as poolModern from './pool-20260920/modern-engine.js';
import * as poolHistory from './pool-20260920/history-engine.js';
import * as poolPreview from './pool-20260920/preview.js';
import * as finalModern from './roster-20260920/modern-engine.js';
import * as finalHistory from './roster-20260920/history-engine.js';
import * as finalPreview from './roster-20260920/preview.js';
...
import * as modern from './ratings-20260920/modern-engine.js';
import * as history from './ratings-20260920/history-engine.js';
...
const engines = new Map([
 ...[poolModern.CLASSIC_RULES_VERSION,poolModern.PACK_RULES_VERSION].map(v=>[v,poolModern]),
 ...[poolHistory.CLASSIC_RULES_VERSION,poolHistory.PACK_RULES_VERSION].map(v=>[v,poolHistory]),
 ...[finalModern.CLASSIC_RULES_VERSION,finalModern.PACK_RULES_VERSION].map(v=>[v,finalModern]),
 ...[finalHistory.CLASSIC_RULES_VERSION,finalHistory.PACK_RULES_VERSION].map(v=>[v,finalHistory]),
...
export function isClassicRulesVersion(version) {
  return [legacy,priorModern,priorHistory,previousModern,previousHistory,septemberModern,septemberHistory,finalModern,finalHistory,poolModern,poolHistory,modern,history].some(e=>e.CLASSIC_RULES_VERSION===version);
}
export function isLegacyRulesVersion(version) { return engines.get(version)===legacy; }
export function legacyView(version) { return isLegacyRulesVersion(version)?legacyPreview:[priorModern,priorHistory].includes(engines.get(version))?priorPreview:[previousModern,previousHistory].includes(engines.get(version))?previousPreview:[septemberModern,septemberHistory].includes(engines.get(version))?septemberPreview:[finalModern,finalHistory].includes(engines.get(version))?finalPreview:[poolModern,poolHistory].includes(engines.get(version))?poolPreview:null; }
...
function poolForRules(version) { return [history,priorHistory,previousHistory,septemberHistory,finalHistory,poolHistory].includes(engines.get(version))?'history':engines.get(version)===legacy?'legacy':'modern'; }
