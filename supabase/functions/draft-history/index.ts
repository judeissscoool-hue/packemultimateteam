...
import { getEngineForRules, rulesForPool } from "../_shared/atu-engine-roster-20260920.js";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const record=async (previous: unknown) => {
      ...
      if(!['atu-classic-v4','atu-history-draft-v2','atu-classic-v5','atu-history-draft-v3','atu-classic-v6','atu-history-draft-v4'].includes(run.rules_version))return;
    ...
    if(![previousVersion,body.pool==='history'?'atu-history-draft-v3':'atu-classic-v5',rulesForPool(body.pool,'draft')].includes(version))return json(origin,400,{error:'Invalid draft rules'});
